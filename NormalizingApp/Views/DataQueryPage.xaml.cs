...
namespace NormalizingApp.Views
{
    ...
    public partial class DataQueryPage : UserControl
    {
        ...
        private List<Models.DataBaseRecord> list = new List<Models.DataBaseRecord>();
        //曲线数据绑定
        ...
        public DataQueryPage()
        {
            InitializeComponent();
            //数据库查询日期改为初始设定的日期
            datePickerCtrl.SelectedDate = ViewModels.LoginWindowViewModel.productNumber.AccessDate;
        }
        ...
        public void SqlInquireRepetition()
        {
            ...
            partNumberlBx.Items.Clear();
            listTem = DataBase.CQServices.GetRecordSql(@"SELECT First(DataSheet.serialNumber) AS [serialNumber], Count(DataSheet.serialNumber) AS 记录总行数
                                                FROM DataSheet
        ...
        public void SqlInquireText(string text)
        {
            ...
            list = DataBase.CQServices.GetRecordBySql(@"SELECT [serialNumber], [operatorName], [jobNumber], [heatTime], [voltage], [current], [power], [frequency], [energy], [temperature1], [temperature2], [temperature3], [coolingTime], [temperature4], [pressure], [flow1], [flow2], [flow3]
                                                    FROM DataSheet
        ...
        private void DatePicker_SelectedDateChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                ...
                {
                    DataBase.DBHelp.FileName = GetFilePath();
        ...
        private void VoltageCurve()
        {
            ...
            CurveClear();//清除曲线
            foreach (Models.DataBaseRecord item in list)
            {
        ...
        private void CurveClear()
        {
            ...
            plotter1.Viewport.Visible = new Rect(0, 0, 0, 0);//初始化坐标
        }
    }
}
